...
export interface YgiModalProps {
  isOpen: boolean;
  setIsOpen: (isOpen: boolean) => void;
}

export default function DepositAndWithdrawModal({isOpen, setIsOpen}: YgiModalProps) {
  const {action, selectedYgi} = useContext(SelectedYgiContext)
  const {vaultAddress, tokenAddress, abi, tokenLogoUrl, decimals} = selectedYgi;
  ...
  const performAction = async (action: TransactionAction) => {
    ...
    posthog?.capture(`TX_MODAL:${action}`, {action, strategy: selectedYgi.name, amount: visibleAmount})
  }
        ...
        >
          ...
          <Box className={`bg-backgroundSecondary rounded-lg`}>
            <div className="pt-5 p-4 flex items-center">
              ...
              <span className="ml-3 text-3xl">{selectedYgi.tokenSymbol}</span>
