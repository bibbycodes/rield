...
describe("HOP ERC20 Strategy", () => {
  async function setupFixture() {
    ...
    const hopTokenVariant = await Token.deploy("hUSDC", "hUSDC", 6) as TokenMock;
    await hopTokenVariant.deployed();

    const lpToken = await Token.deploy("USDC-hUSDC", "USDC-hUSDC-LP", 18) as TokenMock;
    ...
    const hopTrackerMock: HopTrackerMock = (await HopTrackerMock.deploy(usdcToken.address, hopTokenVariant.address, lpToken.address)) as HopTrackerMock;
    ...
    const vault: RldTokenVault = (await Vault.deploy('RLD_TOKEN', 'RLD Token Vault LP')) as RldTokenVault;
    ...
    await usdcToken.mintFor(hopTrackerMock.address, ONE_THOUSAND_USDC);
    ...
    await hopTokenVariant.mintFor(hopTrackerMock.address, ONE_THOUSAND_USDC);
    await hopTokenVariant.mintFor(hopPoolMock.address, ONE_THOUSAND_USDC);
  ...
  describe("Deposit", () => {
    it("Depositing into vault sends want amount to strategy and stakes into pool under the strategy's address, mints token to alice", async () => {
      const {alice, vault, strategy, hopTracker, usdcToken} = await loadFixture(setupFixture);
      ...
      expect(await hopTracker.deposits(strategy.address)).to.equal(ONE_USDC);
    ...
    it("Mints token for bob and alice in proportion to their deposits", async () => {
      const {alice, bob, vault, strategy, hopPool, usdcToken} = await loadFixture(setupFixture);
      await usdcToken.connect(alice).approve(vault.address, TEN_USDC);
      await vault.connect(alice)
        .deposit(ONE_USDC);
    ...
    it("Deposits are disabled when the strat is stopped", async () => {
      const {alice, vault, strategy, usdcToken, hopPool} = await loadFixture(setupFixture);
      ...
      await expect(await usdcToken.allowance(strategy.address, hopPool.address)).to.equal(0);
    })
  ...
  describe("Harvest", () => {
    it("Compounds, claims and restakes, owner takes 30% of pf, rest goes back into strategy", async () => {
      const {alice, vault, strategy, hopPool, hopTracker, usdcToken, deployer} = await loadFixture(setupFixture);
      await usdcToken.connect(alice).approve(vault.address, TEN_USDC);
      await vault.connect(alice).deposit(ONE_USDC);
  ...
  describe("Utils", () => {
    describe("Pausing and un-pausing", () => {
      ...
      it("Deposits are disabled when the strat is stopped", async () => {
        ...
        await strategy.stop();
        await usdcToken.connect(alice).approve(vault.address, TEN_USDC);
        const depositTx = vault.connect(alice).deposit(ONE_USDC)
